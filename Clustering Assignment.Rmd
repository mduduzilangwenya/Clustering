...
library(caret)
confusionMatrix(diabetes_kmeans$Outcome, diabetes_kmeans$cluster)
...
diabetes_hclust$OutcomeClust <- grouping
...
#confusion Matrix
confusionMatrix(diabetes_hclust$Outcome, diabetes_hclust$OutcomeClust)
```
...
dunn(dhclust.norm, average)
#memb2
complete <- cutree(hc4, k = 8)
dunn(dhclust.norm, complete)
#memb4
Ward <- cutree(hc6, k = 10)
dunn(dhclust.norm, Ward)
#memb6
```
...
diabeteshclust_new <- diabetes %>% mutate(HierCluster = average, HierCluster2 = complete,HierCluster3 = Ward,outcome = diabetes.copy$Glucose)
...
ggplot(diabeteshclust_new, aes(x = as.factor(HierCluster), y = Glucose)) +
  geom_boxplot(colour = "black") +
  theme + labs(title = "Distribution of Glucose by cluster of average method", x = "cluster", y = "Glucose")

ggplot(diabeteshclust_new, aes(x = as.factor(HierCluster2), y = Glucose)) +
  geom_boxplot(colour = "black") +
  theme + labs(title = "Distribution of Glucose by cluster of complete method", x = "cluster", y = "Glucose")

ggplot(diabeteshclust_new, aes(x = as.factor(HierCluster3), y = Glucose)) +
  geom_boxplot(colour = "black") +
